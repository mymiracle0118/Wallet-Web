...
import * as useNavigateHooks from 'lib/woozie'
...
describe('<ChangePassword />', () => {
  beforeEach(() => {
    ...
    jest.spyOn(useNavigateHooks, 'useNavigate').mockImplementation(() => ({
      navigate: jest.fn(),
    }))
  ...
  test('should successfully change password', async () => {
    ...
    await userEvent.type(screen.getByPlaceholderText('Current Password'), 'password')
    await userEvent.type(screen.getByPlaceholderText('New Password'), 'password-new')
    await userEvent.type(screen.getByPlaceholderText('Confirm New Password'), 'password-new')
    await userEvent.click(screen.getByTestId('button-change'))
  ...
  test('should show invalid password', async () => {
    ...
    await userEvent.type(screen.getByPlaceholderText('Current Password'), 'password-bad')
    await userEvent.type(screen.getByPlaceholderText('New Password'), 'password-new')
    await userEvent.type(screen.getByPlaceholderText('Confirm New Password'), 'password-new')
    await waitFor(() => userEvent.click(screen.getByTestId('button-change')))
    await screen.findByTestId('progress-loading').then((el) => expect(el).toBeInTheDocument())
    await screen
      .findByText('Error: invalid password', {}, { timeout: 30000 })
      .then((el) => expect(el).toBeInTheDocument())
  })

  test('should show password does not match', async () => {
    render(
      <I18nextProvider i18n={i18n}>
        <ChangePassword />
      </I18nextProvider>
    ...
    await userEvent.type(screen.getByPlaceholderText('Current Password'), 'password-bad')
    await userEvent.type(screen.getByPlaceholderText('New Password'), 'password-new')
    await userEvent.type(screen.getByPlaceholderText('Confirm New Password'), 'password-mismatch')

    await screen.findByText('passwords must match', {}, { timeout: 30000 }).then((el) => expect(el).toBeInTheDocument())
  })
})
